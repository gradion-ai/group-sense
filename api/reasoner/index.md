...
```
get_new_messages() -> list[dict[str, Any]]
```

Return new data since last save for incremental persistence.

Returns an entry containing the new messages from the last process() call along with the cumulative processed count. Clears the internal new messages buffer after returning.

Returns:

| Type                   | Description                                                                                                                                              |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `list[dict[str, Any]]` | List containing a single entry with 'messages' (new conversation messages) and 'processed' (cumulative message count), or empty list if no new messages. |
...
```
set_serialized(lines: list[dict[str, Any]])
```

Restore the reasoner's state from JSONL lines.

Reconstructs the conversation history by concatenating messages from all entries. Uses the processed count from the last entry.

Parameters:

| Name    | Type                   | Description                                                                                                     | Default    |
| ------- | ---------------------- | --------------------------------------------------------------------------------------------------------------- | ---------- |
| `lines` | `list[dict[str, Any]]` | List of entries, each containing 'messages' (conversation messages) and 'processed' (cumulative message count). | *required* |
